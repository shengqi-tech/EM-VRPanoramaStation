...
export type DispatchLogDrawerProps = {
  job?: API.XxlJobInfoVo;
};

const DispatchLogDrawer = React.forwardRef((props: DispatchLogDrawerProps, ref: any) => {
  ...
  // const jobH = job
  //   ? JSON.parse(job?.jobGroupVo?.jobHandlers)?.find((jobHandler) => {
  //       return jobHandler.jobhName == job?.executorHandler;
  //     })
  //   : null;
  ...
  const ProDescriptionsColumns: ProDescriptionsItemProps<API.XxlJobLogVo>[] = [
    {
      title: '客户名称',
  ...
  return (
    <div>
      <DrawerForm<API.XxlJobGroupVo>
        ...
        title={`${job?.jobGroupVo?.customerVo?.ems_customer_name}调度日志`}
        // getContainer={false}
        // closable={false}
